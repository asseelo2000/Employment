...
class AbstractTicketForm(CustomFieldMixin, forms.Form):
    ...
    def _create_ticket(self):
        ...
        print(f"Title: {ticket.queue.title}, Ticket Created With Queue ID: {ticket.queue.id}")
    ...
    def _send_messages(ticket, queue, followup, files, user=None):
        ...
        if ticket.assigned_to and ticket.assigned_to.usersettings_help_desk.email_on_ticket_assign:
...
class TicketForm(AbstractTicketForm):
    ...
    origin_url = forms.URLField(required=False, widget=forms.HiddenInput)

    submitter_username = forms.CharField(required=False, widget=forms.HiddenInput)
    
    submitter_phone = forms.CharField(required=False, widget=forms.HiddenInput)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and user.is_authenticated:
            ...
            self.initial['submitter_username'] = user.username
            self.initial['submitter_phone'] = user.phone_number if hasattr(user, 'phone_number') else "Not Available"
    ...
    def save(self, user):
        ...
        # if self.cleaned_data.get('submitter_email'):
        #    et.save()
        
        # if self.cleaned_data.get('submitter_username'):
        #     ticket.submitter_username = self.cleaned_data['submitter_username']
        # ticket.save()

        # if self.cleaned_data.get('submitter_phone'):
        #     ticket.submitter_phone = self.cleaned_data['submitter_phone']
        # ticket.save() ticket.submitter_email = self.cleaned_data['submitter_email']
        # tick

        # if self.cleaned_data.get('origin_url'):
        #     ticket.submitted_from_url = self.cleaned_data['origin_url'] 
        # ticket.save()

        if user and user.is_authenticated:
            ticket.submitter_email = self.cleaned_data.get('submitter_email') or user.email
            ticket.submitter_username = user.username  # Directly from User model
            ticket.submitter_phone = user.phone_number if hasattr(user, 'phone_number') else "Not Available"
            ticket.ticket_type = self.cleaned_data['ticket_type']
        else:
            ticket.submitter_email = self.cleaned_data.get('submitter_email') or "anonymous@example.com"
            ticket.submitter_username = self.cleaned_data.get('submitter_username') or "Anonymous"
            ticket.submitter_phone = self.cleaned_data.get('submitter_phone') or "Not Available"
            ticket.ticket_type = self.cleaned_data.get('ticket_type') or "Not Specified"
