...
class Queue(models.Model):
    ...
    is_default = models.BooleanField(
        _('Is Default'),
        default=False,
        help_text=_('If this Queue is set as default, it will be used for '
                    'all new tickets.'),
        null=True,
        blank=True,
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id:  # Only for new Queue objects
            # Prepare the permission associated to this Queue
            ...
            try:
                ...
                print(f"Permission '{basename}' already exists for Queue '{self.title}'. Updating name.")
                permission.name = _("Permission for queue: ") + self.title
                permission.save()
            except Permission.DoesNotExist:
                # No existing permission, create a new one
                try:
                    ...
                    print(f"Created new permission '{basename}' for Queue '{self.title}'.")
                except IntegrityError as e:
                    print(f"Failed to create permission for Queue '{self.title}': {e}")
                    raise
    ...
    def delete(self, *args, **kwargs):
        ...
        # Once the Queue is safely deleted, remove the associated permission (if it exists)
        if permission_name:
            try:
                # permission_name is in the format 'helpdesk.queue_access_<slug>'
                # Extract the codename (e.g., 'queue_access_support')
                codename = permission_name.split('.', 1)[1] if '.' in permission_name else permission_name
                permission = Permission.objects.get(codename=codename)
                permission.delete()
                print(f"Deleted permission '{codename}' for Queue '{self.title}'.")
            except Permission.DoesNotExist:
                print(f"Permission '{codename}' not found for Queue '{self.title}'. No deletion needed.")
            except Exception as e:
                print(f"Error deleting permission '{codename}' for Queue '{self.title}': {e}")
...
class Company(models.Model):
    ...
    company_id = models.CharField(max_length=50, unique=True, help_text="Unique identifier used in API submissions")
    feedback_url = models.URLField(help_text="URL to send feedback to (e.g., http://employment2.com/feedback/)")
...
class Ticket(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.SET_NULL, null=True, blank=True, help_text="Company submitting this ticket")

    external_ticket_id = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        verbose_name="External Ticket ID",
        help_text="ID of the ticket in the external support system."
    )


    TICKET_TYPE_CHOICES = (
        ('issue', _('Issue')),
        ('development_note', _('Development Note')),
    )
    ...
    ticket_type = models.CharField(
        ...
        default='issue',
        help_text=_('Type of the ticket: Issue or Development Note.'),
    )
    ...
    submitter_username = models.CharField(
        _('Submitter Username'),
        max_length=150,
        blank=True,
        null=True,
        help_text=_('Username of the submitter.'),
    )
    ...
    submitted_from_url = models.URLField()
    ...
    def get_submitter_username(self):
            user = self.get_submitter_userprofile()
            return user.username if user else "Unknown"

    def get_submitter_phone(self):
        user = self.get_submitter_userprofile()
        return user.phone_number if user and hasattr(user, 'phone_number') else "Not Available"
...
class UserSettings(models.Model):
    ...
    user = models.OneToOneField(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="usersettings_help_desk")
