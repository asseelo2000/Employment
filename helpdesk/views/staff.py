...
import typing
from helpdesk.serializers import TicketSerializer, FollowUpSerializer
...
#     ).order_by('username')[:20]  # Limit to 20 results for performance
#     results = [
#         {'id': user.id, 'text': user.get_username()}
#         for user in users
#     ]
...
def send_feedback(request):
    ...
    try:
        ...
        if not ticket.company or not ticket.company.feedback_url:
            return Response({"error": "No company or feedback URL associated with this ticket."}, status=status.HTTP_400_BAD_REQUEST)
        
        feedback_data = {
            "ticket_id": ticket_id,
            "message": message,
        }
        response = requests.post(ticket.company.feedback_url, json=feedback_data, timeout=5)
        response.raise_for_status()
        
        FollowUp.objects.create(
            ticket=ticket,
            user=request.user,
            title=f"Feedback sent to {ticket.company.name}",
            comment=message,
            public=False
        )
        return Response({"status": "Feedback sent successfully"}, status=status.HTTP_200_OK)
...
def dashboard(request):
    ...
    if request.user.is_authenticated and hasattr(request.user, 'usersettings_help_desk'):
        tickets_per_page = request.user.usersettings_help_desk.tickets_per_page
    else:
        tickets_per_page = 25
...
def mass_update(request):
    ...
    selected_user_id = request.POST.get('selected_user', None)
    ...
    if action == 'assign_select2' and selected_user_id:
        try:
        ...
        action = 'assign'
    elif action == 'kbitem_none':
        action = 'set_kbitem'
    elif action.startswith('kbitem_'):
        parts = action.split('_')
    ...
    for t in Ticket.objects.filter(id__in=tickets):
        ...
        if action == 'assign' and t.assigned_to != user:
            ...
            if t.assigned_to and t.assigned_to.usersettings_help_desk.email_on_ticket_change:
...
def ticket_list(request):
    ...
    if not is_supervisor:
        # Normal users only see tickets they submitted
        query_params['filtering']['submitter_email'] = request.user.email
    ...
    return render(request, 'helpdesk/ticket_list.html', dict(
        ...
        default_tickets_per_page=request.user.usersettings_help_desk.tickets_per_page,
...
class CreateTicketView(MustBeStaffMixin, abstract_views.AbstractCreateTicketMixin, FormView):
    ...
    def form_valid(self, form):
        try:
            ...
            # Set submitter_email from the authenticated user if not provided
            if self.request.user.is_authenticated and not form.cleaned_data.get('submitter_email'):
                form.cleaned_data['submitter_email'] = self.request.user.email
                

            # Notify supervisors via email
            supervisors = User.objects.filter(groups__name='Supervisors')
            if not supervisors:
                messages.error(self.request, 'No supervisors available to review your ticket.')
            else:
                send_mail(
                    'New Internal Ticket Submitted',
                    f'A new ticket "{self.ticket.title}" has been submitted by the user {self.request.user.username} for review.\n'
                    f'Description: {self.ticket.description}\n'
                    f'View it at: https://support.togits.com{self.ticket.get_absolute_url()}',
                    settings.DEFAULT_FROM_EMAIL,
                    [supervisor.email for supervisor in supervisors],
                    fail_silently=True,
                )
                # Create admin notifications for supervisors
                # for supervisor in supervisors:
                #     Notification.objects.create(
                #         user=supervisor,
                #         title=f'New Ticket: {self.ticket.title}',
                #         message=f'Ticket #{self.ticket.id} submitted by {self.request.user.username}. Description: {self.ticket.description[:100]}...',
    ...
    def get_success_url(self):
        ...
        # Check if the request came from the admin site
        if 'admin' in request.path or 'admin' in referer:
            if referer:
                return referer  # Go back to exact admin page user came from
            return reverse('admin:index')

        # If ticket exists and user can access the queue
        if hasattr(self, 'ticket') and self.ticket and HelpdeskUser(request.user).can_access_queue(self.ticket.queue):
            return self.ticket.get_absolute_url()

        # Default fallback to helpdesk dashboard
