...
class Select2Mixin:
    ...
    def media(self):
        ...
        extra = "" if settings.DEBUG else ".min"  # NOQA
        select2_js = tuple(settings.SELECT2_JS) if settings.SELECT2_JS else ()
        select2_css = tuple(settings.SELECT2_CSS) if settings.SELECT2_CSS else ()
...
class Select2AdminMixin:
    ...
    def media(self):
        ...
        css["screen"] += ("django_select2/django_select2.css",)
        return forms.Media(
...
class HeavySelect2Mixin:
    ...
    def build_attrs(self, base_attrs, extra_attrs=None):
        ...
        if self.dependent_fields:
            ...
            ) if  self.inverse is False else " ".join(
                set(self.dependent_fields.values())
            )
